...
> **Pozadí produktu:** IndustrySignal je digitalizační vrstva existující intelligence operace, která již 17 let pracuje s 56 000 průmyslovými firmami (interne pod pracovním názvem *Industry Match*; finální brand je **IndustrySignal**). Existuje **konsolidovaná proprietární databáze** s 12 000+ ověřenými firmami (Svět průmyslu inzertní + Raynet CRM + Stage CEO C-level vrstvy). Vedení již dodává placené Supplier Intelligence Reporty manuálně. Portal **NENÍ MVP nového produktu** — je to automatizační vrstva nad existující službou + nový kanál pro self-service Concierge.

**Datová hierarchie (kritická — platí v celém dokumentu):**

```
  Vrstva 1 — PROPRIETÁRNÍ DB (primary source of truth)
             Konsolidovaná z: Svět průmyslu (17 let) + Raynet CRM + Stage CEO
             12 000+ firem s ověřenými vztahy, C-level kontakty, NACE,
             inzertní historie, dodavatelské vazby na OEM

  Vrstva 2 — ENRICHMENT (validation + augmentation)
             ARES, Justice (sbírka listin), Insolvenční rejstřík,
             Clay.com (LinkedIn + role), GLEIF (LEI)
             Nepoužít jako primární zdroj entit — ověřují a obohacují
             vrstvu 1.

  Vrstva 3 — KLIENTSKÁ KOOPERATIVA (network effect, viz §2.5)
             Klienti nahrávají anonymizovaná data → obohacuje sdílenou DB
             → víc dat = lepší signal = víc klientů. Flywheel.

  Vrstva 4 — SIGNAL (real-time monitoring)
             GDELT, RSS hlavního byznys tisku, vanity publishers (§15.5–7),
             customs flow, tendry, news NER
```
...
|---|---|---|
| **Auth** | Magic-link přihlášení, seat-based teams, role (admin/analyst/viewer) | vlastní DB |
| **Aktuální report (SIR)** | On-demand Supplier Intelligence Report dle segmentu — 5 dní manuálně → cíl 1 den s automatizací | Vrstva 1 + 2 + LLM synthesis |
| **Archiv** | Stažitelné PDF historických SIR | R2 |
| **Watch List** | Klient si přidá firmy/segmenty/regiony; daily snapshot | Vrstva 1 + 2 |
| **Alerty** | Push notifikace na změny ve Watch Listu | diff engine + Postmark |
| **SRSC (Target Buyer Intelligence Matrix)** | 6-dimenzionální skórování: konc. riziko, kapac. napětí, fin. expozice, geo riziko, alt. dodavatelé, data score | Vrstva 1 + 2 + ord. škály |
| **Export Map** | Export teplotní mapa CZ→EU dle HS kódů | Celní správa + Eurostat Comext |
| **Data Cooperative (viz §2.5)** | Klient nahrává anonymizovaná data → sdílená DB | klient + redaction pipeline |

**Non-functional požadavky:**
- Český jazyk primárně, EN sekundárně (i18n implementován)
- WCAG AA kontrast (editorial téma)
- Mobile responsive read-only
- GDPR — data klientů v EU, právo zapomenutí, audit log
- 99.5% uptime
- Page load < 2s na P75
- **Kritické USP:** *žádná generovaná ani halucinovaná data* — každé tvrzení/vazba ve výstupu má dohledatelný zdroj. Platí striktně napríč celém produktem (LLM výstupy vždy s evidence_excerpt, §15.8).
...
## 2.5 Data Cooperative — klientské uploady jako network-effect vrstva

> Strategická diferenciace. Klienti mohou do platformy nahrávat **anonymizovaná data**, která se po zpracování stávají součástí sdílené databáze. Více klientů = víc dat = lepší signál pro všechny = víc klientů. Tohle je flywheel, který odděluje IndustrySignal od jakéhokoli konkurenta operujícího jen na free/placených zdrojích.

### Co klienti nahrávají (use case examples)

| Typ dat | Forma uploadu | Anonymizační požadavek | Co z toho |
|---|---|---|---|
| **Seznam dodavatelů** | CSV / XLSX export z ERP | Odstranit smluvní podmínky, ceny per faktura | Supplier-customer hrany do entity graphu |
| **Customer concentration** | CSV (% z tržeb top-5 zákazníků, bez jmen) | Plné anonymizace odběratelů | Geografická + segmentová koncentrace agregovaně |
| **Insolvenční expozice** | CSV (IČO + výše pohledávky) | Bez vlastní firmy | Distress signál v reálném čase |
| **Procurement intel** | Free-text (např. "prohráli jsme zakázku X firmě Y v Q3") | LLM redaction PII | Konkurenční dynamika |
| **Pricing benchmarks** | CSV (kategorie + průměrná cena bez kontextu) | Anonymizováno per produkt | Tržní indexy per NACE |
| **Plánované investice** | Free-text + datum | Anonymizovat zdrojovou firmu | Capacity expansion signal layer |

### Architektonický kontrakt

```
┌──────────────────┐    ┌────────────────────┐    ┌─────────────────────┐
│  Client upload   │───▶│  Redaction pipeline│───▶│  Reviewed cooperative│
│  (encrypted at   │    │  (LLM PII detect + │    │  pool (shared DB)    │
│   rest, R2)      │    │   human review)    │    │                      │
└──────────────────┘    └────────────────────┘    └─────────┬───────────┘
                                                            │
                                                            ▼
                                            Enriches všech klientů
                                            (signal layer, SRSC,
                                            entity links)
```

### Pravidla (legal + technical)

1. **Souhlas v Data Sharing Agreement** — povinný add-on k Subscription Agreement. Bez podpisu klient nemůže upload použít.

2. **Redaction pipeline před vstupem do shared pool**:
   - Haiku 4.5 detekuje PII (jména osob, emaily, telefony, SPZ, bankovní čísla, smluvní částky pod prahem)
   - Pravidlový engine maskuje: vlastní IČO klienta, vlastní brand, role-bound identifiers
   - Human review queue v Studio app pro každý upload (povinný step, žádný automatický merge)
   - Audit log: kdo nahrál, kdy, co se zmergovalo, co se odmítlo

3. **Right to revoke** — klient může kdykoliv požádat o odstranění svého příspěvku. Soft-delete v cooperative pool s 30denním cool-down před hard delete (compliance + chance vyřešit dependency v graph).

4. **No re-identification** — TOS striktně zakazuje pokus o re-identifikaci anonymizovaných dat. Smluvní pokuta 250 000 EUR + okamžité ukončení smlouvy.

5. **Audit trail** povinný (GDPR Article 30):
   - Source ID (klient + upload event)
   - Pre-redaction blob (encrypted, retention 18 měsíců, jen pro audit při sporu)
   - Post-redaction merged into pool (forever, basis pro produkt)
   - Reviewer decision + reasoning

6. **Reward mechanism** — klienti kteří přispěli nezasokrytě (≥ 4 uploady/rok s ≥ 80 % approval rate) dostanou:
   - Priority access k novým Mission výsledkům
   - 10 % sleva na renewal
   - **Contributor Badge** v jejich profilu v portálu (interní recognition)

7. **Reciprocity transparency** — klient v portálu vidí dashboard: "Přispěli jste 12 uploady → benefitujete z 3 800 link hran v graphu od ostatních klientů."

### Datový model (předběžně, dolaďuje se ve Sprintu 8+)

```sql
CREATE TABLE coop_uploads (
  id              UUID PRIMARY KEY,
  org_id          UUID REFERENCES organization(id),
  uploader_user_id UUID REFERENCES users(id),
  upload_type     TEXT,                       -- 'supplier_list','customer_concentration',...
  raw_blob_url    TEXT,                       -- R2 path, encrypted
  redacted_blob_url TEXT,                     -- R2 path po redaction
  redaction_run_id UUID REFERENCES llm_calls(id),
  human_reviewer_id UUID REFERENCES users(id),
  review_decision TEXT,                       -- 'approved','rejected','needs_changes'
  review_notes    TEXT,
  merged_into_pool BOOLEAN DEFAULT FALSE,
  contributed_link_count INT,
  created_at      TIMESTAMPTZ DEFAULT now(),
  reviewed_at     TIMESTAMPTZ,
  revoked_at      TIMESTAMPTZ                 -- soft delete dle pravidla 3
);

CREATE TABLE coop_contributed_links (
  id              UUID PRIMARY KEY,
  upload_id       UUID REFERENCES coop_uploads(id),
  link_id         UUID REFERENCES entity_links(id),
  contributor_org_id_hash TEXT,               -- hash, ne plain ID (anti re-identify)
...
- **NENÍ to LinkedIn / Glassdoor model** — žádné user-generated review obsah, jen strukturovaná business data
- **NENÍ to social network** — firmy se nepropojují přímo, jen přispívají do anonymního pool
- **NENÍ to matchmaking** (matchmaking je separate produkt Industry Match Summit, mimo IndustrySignal scope)
- **NENÍ to free service** — Data Cooperative je benefit subscriptionu, ne acquisition funnel

---

## 3. Data sources — odkud brát a jak

> **Hierarchie zdrojů (viz §1):**
> 1. **Proprietární DB** — primary, source of truth. 12 000+ ověřených firem napříč Svět průmyslu / Raynet / Stage CEO.
> 2. **Enrichment vrstva** — ARES, Justice, Insolvenční rejstřík, Clay, GLEIF. **Není primárním zdrojem entit**, slouží k ověření a obohacení vrstvy 1.
> 3. **Klientská kooperativa** — viz §2.5 výše. Network effect layer.
> 4. **Signal vrstva** — GDELT, tisk RSS, vanity publishers (§15.5–7), customs, tendry. Real-time monitoring.
>
> Tabulky níže popisují zejména vrstvy 2 a 4. Vrstva 1 (interní DB) je vlastní operace — Claude Code dostane connection string + read-only access, ne implementaci scrape connectoru.
...
### 3.2 Firemní data — ENRICHMENT vrstva (Watch List + SRSC validation)

> **Tyto zdroje NEJSOU primární.** Primární zdroj entit = proprietární DB (viz §1). ARES, Justice atd. slouží k **ověření a obohacení** existujících záznamů: aktuální statutáři, finanční výkazy, insolvenční status, NACE klasifikace. Workflow je vždy: *interní záznam → enrichment lookup → cross-check → merge zpět*.

| Zdroj | Cena | Přístup | Frekvence | Použít na |
|---|---|---|---|---|
| **ARES** (wwwinfo.mfcr.cz/ares) | zdarma | REST API (XML/JSON) | denně | Validace IČO, sídla, statutárů, NACE u záznamů v interní DB |
| **Veřejný rejstřík (justice.cz)** | zdarma | scrape (Playwright) | denně | Účetní závěrky, změny ve statutárech, insolvence |
| **Insolvenční rejstřík (isir.justice.cz)** | zdarma | REST API + RSS | denně | **KRITICKÉ** pro Fin. expozice dimenzi v SRSC — návrhy, úpadky |
| **Clay.com** (clay.com) | €€ (~$200/měs starter) | REST API | denně | LinkedIn pozice C-level, aktualizace rolí, emaily |
| **Magnusweb / Bisnode** | €€€ (od ~50k Kč/rok) | REST API | denně | Skóre platební morálky, UBO, credit limity |
...
| **GLEIF** (gleif.org) | zdarma | REST API | denně | LEI lookup pro cross-country entity resolution (§15.9 L1) |

**Pro MVP:** ARES + Justice + Insolvenční rejstřík + Clay.com (vystačí pro ověření 12k+ záznamů v interní DB). Magnusweb/Bisnode přidat s prvním klientem z finančního sektoru.
...
### Auth
- **Magic-link email** (Postmark v prod, mock provider v dev) — žádná hesla
...
### Org / seats model
```
organization (Klient s.r.o.)
  ├── seats: 5 zakoupených, 3 aktivní
  ├── users (každý s rolí: admin | analyst | viewer)
  └── subscription: active | trialing | cancelled | past_due
```

Seat default = 5. Add-on seats v rámci ročního kontraktu (prorated).

### Billing
- **Stripe** (eu_vat povinný — ISDOC export pro velké korporátní klienty)
- **Roční fakturace, jeden tier:**
  - **IndustrySignal Annual** = **12 000 EUR / rok** za organization
  - Zahrnuje: 5 seats, unlimited watch list, unlimited alerts, 2× ročně SIR pro definovaný segment, full portal access, dedikovaný account manager pro Q-onboarding
  - Dodatečné seats: +400 EUR/seat/rok (prorated)
  - Dodatečný on-demand SIR (mimo zahrnuté 2× ročně): cena nestanovena standardízovaně — negotiated per request
- Faktury v EUR + CZK volba, ISDOC dle potřeby
- Trial: 30 dní plný access, bez automatického účtování
...
# Část III — SRSC (Target Buyer Intelligence Matrix) jako canonical schema

> **Tato sekce je závazná pro implementaci SRSC view a datového modelu.** Schéma vychází z 6-dimenzionálního scoring frameworku, který existuje v produktové operaci a je validovaný klienty. Není teoretický návrh.

## §22. Target Buyer Intelligence Matrix — schema, škály, implementace

### Šest dimenzí

| ID | Dimenze | Co měří |
|---|---|---|
| **conc** | Konc. riziko | Míra závislosti na omezeném počtu dodavatelů v kritické kategorii. Vysoká koncentrace = přímé ohrožení výroby při výpadku. |
| **kapac** | Kapac. napětí | Průměrné vytížení klíčových dodavatelů v segmentu. Nad 80 % = omezená flexibilita při urgentní poptávce. |
| **fin** | Fin. expozice | Finanční zranitelnost dodavatelského řetězce — zadluženost, platební morálka, stabilita klíčových kooperantů. |
| **geo** | Geo riziko | Geografická koncentrace dodávek. Závislost na jediném regionu nebo zemi zvyšuje logistické a geopolitické riziko. |
| **alt** | Alt. dodavatelé | Dostupnost ověřených alternativních zdrojů v databázi. Nízká dostupnost = nutnost aktivního sourcingu. |
| **score** | Data score | Kvalita a úplnost datového záznamu firmy v interní databázi (0–100 %). Vyšší score = spolehlivější analýza. |

### Ordinální škály (NE číselné, záměrně)

| Dimenze | Hodnoty | Visual treatment v portálu |
|---|---|---|
| conc, kapac, fin, geo | `KRIT.` / `VYSOKÉ` / `STŘED.` / `NÍZKÉ` | semantic colors: signal-down / signal-warn / signal-info / signal-up |
| alt | `OMEZ.` / `STŘED.` / `DOBRÉ` | semantic colors: signal-down / signal-warn / signal-up |
| score | `0–100 %` (integer) | mono numeric, accent amber pro >80 %, fg-tertiary pro <60 % |

**Proč ordinální škály a ne 0–100 v každé dimenzi:** klient (CPO / CFO / CEO) čte matrix v 5 sekundách. Ordinální se čte rychleji než číslo. Číslo svádí k falešné přesnosti. To je dlouhodobý design call, respektuj ho.

### Priority tag (per row, derivovaný z 6 dimenzí)

| Tag | Pravidlo | Akce |
|---|---|---|
| `TIER-1` | ≥ 2 dimenze `KRIT.` nebo `VYSOKÉ` AND data_score ≥ 85 % | Okamžitý sourcing action — alert na klientském dashboardu |
| `AKTIVNÍ` | data_score ≥ 70 % AND alespoň 1 `KRIT.` nebo `VYSOKÉ` | Sleduje se v Watch Listu, alerty na změny |
| `SUMMIT` | data_score 50–70 % | Indikace o příležitosti (matchmaking offline, mimo portal) |
| `LATENT` | data_score < 50 % | Nedostatečná data — primárně enrichment task před scoring použitelný |

> *Pozn.: `SUMMIT` tag je interní marker pro offline matchmaking events. Portál nezobrazuje matchmaking funkčnost (viz §1 — Summit je separate produkt).*

### Datový model

```sql
CREATE TABLE srsc_scores (
  id              UUID PRIMARY KEY,
  entity_id       UUID REFERENCES entities(id),
  org_id          UUID REFERENCES organization(id),   -- per-klient view (jejich watch list)
  segment_nace    TEXT,
  conc            srsc_level NOT NULL,                -- enum: 'KRIT','VYSOKE','STRED','NIZKE'
  kapac           srsc_level NOT NULL,
  fin             srsc_level NOT NULL,
  geo             srsc_level NOT NULL,
  alt             srsc_alt   NOT NULL,                -- enum: 'OMEZ','STRED','DOBRE'
  data_score      INT NOT NULL CHECK (data_score BETWEEN 0 AND 100),
  priority_tag    TEXT,                               -- 'TIER-1','AKTIVNI','SUMMIT','LATENT'
  computed_at     TIMESTAMPTZ NOT NULL,
  computed_by     TEXT,                               -- 'auto-v1' / 'human-override:<user_id>'
  override_reason TEXT,
  source_evidence_ids UUID[]                          -- pole odkazů na link_evidence
);

CREATE TYPE srsc_level AS ENUM ('KRIT','VYSOKE','STRED','NIZKE');
CREATE TYPE srsc_alt   AS ENUM ('OMEZ','STRED','DOBRE');

CREATE INDEX idx_srsc_org_priority ON srsc_scores(org_id, priority_tag);
CREATE INDEX idx_srsc_entity ON srsc_scores(entity_id);
```

### Auto-computation rules (Sprint 5+)

Pro každou dimenzi pravidlový engine (NE LLM — kvůli auditovatelnosti):

```typescript
// packages/srsc/compute.ts
function computeConcRisk(entity: Entity, context: SrscContext): srsc_level {
  const supplierCount = context.alternativeSuppliers.length;
  const topSupplierShare = context.topSupplierExposurePct;
  if (supplierCount <= 1 || topSupplierShare >= 70) return 'KRIT';
  if (supplierCount <= 3 || topSupplierShare >= 50) return 'VYSOKE';
  if (supplierCount <= 6 || topSupplierShare >= 30) return 'STRED';
  return 'NIZKE';
}

function computeKapacTension(entity: Entity, context: SrscContext): srsc_level {
  const avgUtilization = context.segmentAvgUtilizationPct;
  if (avgUtilization >= 85) return 'KRIT';
  if (avgUtilization >= 75) return 'VYSOKE';
  if (avgUtilization >= 60) return 'STRED';
  return 'NIZKE';
}

function computeFinExposure(entity: Entity, context: SrscContext): srsc_level {
  // ISIR insolvenční signál = okamžitě KRIT
  if (context.activeInsolvencyEvents.length > 0) return 'KRIT';

  // Justice.cz účetní výkazy
  if (context.equityRatio < 0.10 || context.daysPayableOutstanding > 90) return 'VYSOKE';
  if (context.equityRatio < 0.20 || context.daysPayableOutstanding > 60) return 'STRED';
  return 'NIZKE';
}

// ... a tak dále pro geo, alt, data_score
```

Pravidlové enginy jsou v `packages/srsc/`, verzované (`v1`, `v2`...). Změna pravidla = nová verze + bulk re-compute + audit log starých vs nových hodnot.

### Human override workflow

Analyst může dimenzi přepsat:
1. V SRSC view klikne na cell → otevře side panel s pravidlovou logikou + návrhem
2. Vybere novou hodnotu + povinný free-text důvod
3. Uloží se s `computed_by='human-override:<user_id>'` a `override_reason`
4. Audit log + email notification na ostatní analysty v org
5. Auto-recompute v dalším cyklu **neprepíše** override (manual lock); analyst musí explicitně "release override" pro re-engagement auto-pravidel.

### Refresh frequency

- **Auto-compute denně** v noci (Inngest cron, 02:00 UTC) pro všechny entities ve Watch Listech aktivních organizací
- **On-demand** při: novém insolvenčním záznamu (ISIR webhook), novém Justice.cz filing, change v interní DB, klientský upload (§2.5)
- **Refresh per Mission** (§13) — když Mission běží, SRSC se počítá pro každou entitu v graphu (deferred queue)

### Co tahle sekce **NENÍ**

- **NENÍ celých 100 metrik per firma** (jako některé credit scoring platformy). 6 dimenzí je závazný kontrakt, ne abstrakce. Klient se naučí v 5 minutách, ne 5 hodinách.
- **NENÍ aspirační framework** — je to už dnes manuálně produkovaný výstup, který klienti platí za 3 500 EUR / SIR. Implementace je automatizace existujícího, ne research.
- **NENÍ post-hoc škálovatelná na všechny segmenty bez kalibrace** — pravidlové prahy (70 %, 50 %, atd.) jsou kalibrované pro CZ industrial segmenty. Pro novou geografii/segment musí editor zkalibrovat prahy s reálnými daty před release.
